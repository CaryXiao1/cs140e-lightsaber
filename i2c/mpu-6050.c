...
int accel_has_data(const accel_t *h) {
    // pg. 29 of reg map, see DATA_RDY_INT at bit 0
    char reg = imu_rd(h->addr, INT_STATUS);
    return bit_is_on(reg, 0);
}
...
// https://stackoverflow.com/questions/60419390/mpu-6050-correctly-reading-data-from-the-fifo-register
    ...
    uint8_t accel_config = imu_rd(addr, accel_config_reg); 
    accel_config &= ~(0b11 << 3); 
    // accel_config = bits_set(accel_config, 3, 4, accel_g);  
    imu_wr(addr, accel_config_reg, accel_config);
...
void mpu6050_reset(uint8_t addr) {
    ...
    // todo("reset device");
    uint8_t pwr_mgmt_reg = imu_rd(addr, PWR_MGMT_1); 
    pwr_mgmt_reg = pwr_mgmt_reg | (1 << 7); // page 42; resets all internal registers to default values
    imu_wr(addr, PWR_MGMT_1, pwr_mgmt_reg); 

    // TODO: See Section 3 for default values
    ...
    pwr_mgmt_reg = imu_rd(addr, PWR_MGMT_1); // Had to read again
    pwr_mgmt_reg = bit_clr(pwr_mgmt_reg, 6); // pg. 42
    imu_wr(addr, PWR_MGMT_1, pwr_mgmt_reg); // put back in memory
    ...
    uint8_t usr_ctrl_reg = imu_rd(addr, USER_CTRL); 
    usr_ctrl_reg = bits_set(usr_ctrl_reg, 0, 2, 0b111); // the signal path
    imu_wr(addr, USER_CTRL, usr_ctrl_reg); 
    ...
    // - read to clear it.
    uint8_t int_pin_cfg_reg = imu_rd(addr, INT_PIN_CFG); 
    int_pin_cfg_reg = bit_set(int_pin_cfg_reg, 4); 
    int_pin_cfg_reg = bit_set(int_pin_cfg_reg, 5); // NOTE: questionable
    imu_wr(addr, INT_PIN_CFG, int_pin_cfg_reg); 

    uint8_t int_enable_reg = imu_rd(addr, INT_ENABLE); 
    int_enable_reg = bit_set(int_enable_reg, 0);  // data ready interrupt
    // int_enable_reg = bit_set(int_enable_reg, 3); 
    // int_enable_reg = bit_set(int_enable_reg, 4); 
    // int_enable_reg = bit_set(int_enable_reg, 6); 
    imu_wr(addr, INT_ENABLE, int_enable_reg); 
...
gyro_t mpu6050_gyro_init(uint8_t addr, unsigned gyro_dps) { 
    ...
    uint8_t reg = imu_rd(addr, GYRO_CONFIG); 
    reg = bits_set(reg, 3, 4, gyro_dps); 
    // accel_config = bits_set(accel_config, 3, 4, accel_g);  
    imu_wr(addr, GYRO_CONFIG, reg);
...
int gyro_has_data(const gyro_t *h) {
    uint8_t int_status_register = imu_rd(h->addr, INT_STATUS);
    return bit_is_on(int_status_register, 0); 
    // todo("implement this");
}
